...
interface BlobSink {
     ...
     * 单个附件的实际占盘字节数（密文）。
     *
     * 与 [totalBytes] 是"一个"与"全部"的关系，之所以两个都要：占用明细要按
     * **记录引用的类别**（照片 / 视频 / 没人引用的）分别求和，而总数减不出分类。
     *
     * 默认 0：内存实现没有"文件大小"这个概念，而它只在统计里被读到 ——
     * 返回 0 会让明细里那一项显示成 0，而不是让整个统计崩掉。
     */
    fun sizeOf(blobId: String): Long = 0L

    /**
     * 写入中断留下的半截文件占了多少字节。
     *
     * 它们不满足 [com.fpb.vault.data.RowIds.isValid]，因此 [listIds] 看不见、
     * 孤儿清理也扫不到（只能靠 [purgeStaleTempFiles] 回收）。
     * 占用明细里必须**单独列这一项**，否则"占用空间"会比"各项明细之和"大出一块，
     * 而用户看到这种情况时唯一的疑问恰恰是"多出来的那部分是什么"。
     *
     * 默认 0：内存实现没有"半截文件"。
     */
    fun staleTempBytes(): Long = 0L
