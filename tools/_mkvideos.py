...
## 输出
    ...
    build/videos/landscape.mp4   320×180（16:9），60 秒，**多块**（底部有红噪声，见下）
...
横屏给到 60 秒，是照着走查的实际节奏倒推的：打开影片 → 连拍取"正在播"的证据 →
暂停 → 读数 → 拖进度条 → 恢复播放 → **切后台再回来**（这一段的判据必须在"仍在播"
的状态下做）→ 再抓几帧，全程四十多秒。片子比这个短，后半段就会撞上"刚好播完"，
而"刚好播完"会让"切后台回来画面还在不在"这类断言失去意义 ——
上一版 10 秒就是这么翻的车：检查按钮状态之前片子已经放完，
...
def main():
    ...
    spec = [
        ...
        ("landscape.mp4", 320, 180, 60, 46, 12),
